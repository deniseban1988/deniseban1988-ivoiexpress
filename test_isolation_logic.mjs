import { inferChannelType } from './src/lib/iptv/m3uParser.js';
...
const testCases = [
  ...
  { name: 'Cartoon Network', group: 'JEUNESSE', url: 'http://server.com/live/cn.m3u8', expected: 'TV' }, // Cartoon Network Live is TV
  { name: 'Spider-Man Animation', group: 'CARTOON', url: 'http://server.com/series/spiderman/1.mp4', expected: 'SERIES' }, // If it's a series
];
...
testCases.forEach(tc => {
  ...
  if (pass) passCount++;
  console.log(`[${pass ? 'PASS' : 'FAIL'}] Input: "${tc.name}" | Group: "${tc.group}" | URL: ${tc.url}`);
